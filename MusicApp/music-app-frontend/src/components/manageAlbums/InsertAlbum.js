...
function InsertAlbum() {
  ...
  const handleSubmit = async (e) => {
    ...
    const sanitizedToken = encodeURIComponent(token);
      const response = await fetch('http://localhost:8080/albums/insertAlbum', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setMessage('Album inserted successfully');
        setAlbum({
          albumID: ' ',
          albumTitle: '',
          artist: '',
        ...
        setError('Failed to insert album');
      }
    } catch (error) {
      console.error('Failed to insert album:', error);
      setError('Failed to insert album: ' + error.message);
    }
  };

  return (
    <div className="insertAlbumContainer">
